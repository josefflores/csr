...
function Api( apiPath ) {
    ...
    this.call = function ( order , call , argv ) {
        ...
        //  Generate and store call
        lastCall = [{"order":order,"call":call,"parameter":argv,"time":time}] ;
 ...
 *  @name   getProfile
 *
 *  This function gets the events for the profile
 *
 *  @param  target  the element to insert the events into
 */
var getprofile = function( element ) {

    var val ;

    val = parseInt( $( element ).attr( 'value' ) ) ;
    console.log( val ) ;
    console.log( val + ( 24 * 60 * 60 ) );
    api.call( 1 , 'getWidget' , [ 'event-manager' , val , val + ( 24 * 60 * 60 ) ] ) ;
    ...
    $( element ).closest( "div.profile" ).children( 'div.pane-center' ).html( tmp[2][0]  ) ;

}

var handleFileSelect = function() {
    if( $( '.pane-center-input textarea').val() != '' ) {
        api.call( 1 , 'storeFile' , [ {"MIME":"application/octet-stream" , "DATA":btoa( $( '.pane-center-input textarea').val() ) , "ENCODING": "base64"} ] ) ;
    }
    ...
    $( '.pane-center-input textarea').val('');
 ...
 *  @name collaps
 *
 *  This collapse the timeline according to the selected values
 *
 *  @param  element     The element that was clicked
 *  @param  period      The time period whether it is a year month or day
 */
function collapse(element, period) {

    switch (period) {
        case 'Y':
                $(element).siblings().toggle();                              // Toggle contents of year
                $(element).parent().siblings('li').children('ul').hide();    // Collapse other years

                //    Opens first month
                $(element).parent().children('ul').children('li').children('ul').hide();
                $(element).parent().children('ul').children('li:first-child()').children().show();

            break;
        case 'M':
                //    hides other years
                collapse( $(element).parent().parent().parent().children( 'a:first-child()' ) , 'Y' ) ;
                collapse( $(element).parent().parent().parent().children( 'a:first-child()' ) , 'Y' ) ;

                //console.log(  ) ;//siblings( ) ;
                $(element).siblings().show();                          // Toggle contents of month
                $(element).parent().siblings('li').children('ul').hide();    // Collapse other months
